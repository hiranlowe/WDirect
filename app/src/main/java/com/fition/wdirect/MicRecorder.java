...
public class MicRecorder implements Runnable {
    ...
    MicRecorder(TalkingActivity talkingActivity) {
    ...
    public void run() {
        ...
        try {
            final OutputStream outputStream = ConnectionHandler.getSocket().getOutputStream();

            final byte[] audioBuffer = new byte[bufferSize];


            if (ActivityCompat.checkSelfPermission(talkingActivity, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(talkingActivity, new String[]{Manifest.permission.RECORD_AUDIO}, 1);
            }
